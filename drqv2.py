...
class Critic(nn.Module):
    ...
    def forward(self, obs, action):
        # feat = self.obs_encoder(obs)
        feat_action = torch.cat([obs, action], dim=-1)
...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
        ...
        # models
        self.encoder = FeatureExtractor(obs_shape).to(device)
        self.actor = Actor(self.encoder.feat_dim, action_shape, hidden_dim).to(device)
        self.critic = Critic(self.encoder.feat_dim, action_shape, hidden_dim).to(device)
        self.critic_target = Critic(self.encoder.feat_dim, action_shape, hidden_dim).to(device)
